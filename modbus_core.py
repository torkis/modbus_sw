...
libmodbus = ffi.dlopen("modbus")


def get_float(data):
    return libmodbus.modbus_get_float(data)


def set_float(value, data):
    libmodbus.modbus_set_float(value, data)
...
class ModbusCore(object):
    def _run(self, func, *args):
        ...
        if rc == -1:
            raise Exception(ffi.string(libmodbus.modbus_strerror(ffi.errno)))

    def connect(self):
        return self._run(libmodbus.modbus_connect)

    def set_slave(self, slave):
        return self._run(libmodbus.modbus_set_slave, slave)
    ...
    def read_bits(self, addr, nb):
        ...
        self._run(libmodbus.modbus_read_bits, addr, nb, dest)
        return dest

    def read_input_bits(self, addr, nb):
        dest = ffi.new("uint8_t[]", nb)
        self._run(libmodbus.modbus_read_input_bits, addr, nb, dest)
        return dest

    def read_registers(self, addr, nb):
        dest = ffi.new("uint16_t[]", nb)
        self._run(libmodbus.modbus_read_registers, addr, nb, dest)
        return dest

    def read_input_registers(self, addr, nb):
        dest = ffi.new("uint16_t[]", nb)
        self._run(libmodbus.modbus_read_input_registers, addr, nb, dest)
        return dest

    def write_bit(self, addr, status):
        # int
        self._run(libmodbus.modbus_write_bit, addr, status)

    def write_register(self, addr, value):
        # int
        self._run(libmodbus.modbus_write_register, addr, value)

    def write_bits(self, addr, nb, data):
        # const uint8_t*
        nb = len(data)
        self._run(libmodbus.modbus_write_bits, addr, nb, data)

    def write_registers(self, addr, data):
        # const uint16_t*
        nb = len(data)
        self._run(libmodbus.modbus_write_registers, addr, nb, data)

    def write_and_read_registers(self, write_addr, data, read_addr, read_nb):
        # const uint16_t*
        dest = ffi.new("uint16_t[]", read_nb)
        self._run(
            libmodbus.modbus_write_and_read_registers,
            write_addr,
            len(data),
            data,
            read_addr,
